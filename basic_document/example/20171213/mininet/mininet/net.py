...
class Mininet( object ):
    ...
    def iperf( self, hosts=None, l4Type='TCP', udpBw='10M', fmt=None,
        ...
        output( '*** Results: %s\n' % result )
        return result
    def iperf_single( self,hosts=None, udpBw='100k', period=60, port=5001):
        ...
        #iperfArgs = 'iperf -u'
        iperfArgs = 'iperf -u -p '+str(port)
        bwArgs = '-b ' + udpBw
        cmd=iperfArgs + ' -s -i 1 ' + ' |tee   ./learn/log/server_' + client.name+'-'+server.name+ '.out' + '&'
        print "***start server v7:"+cmd+"***"
        server.cmd(cmd)
        cmd=iperfArgs + ' -t '+ str(period) + ' -c ' + server.IP() + ' ' +\
        bwArgs+' |tee ./learn/log/client_' + client.name+'-'+server.name + '.out'+'&'
        print "***start client v7:"+cmd+"***"
        client.cmd(cmd)
    def iperfMulti(self, bw, period=60):
      ...
      sleep(period)
      print "test has done"
    def iperfRandom(self,_bw, _period):
      base_port = 7001
      host_list = [h for h in self.hosts]
      bw=_bw
      period=_period
      while(True):
          client = random.choice(host_list)
          server = client
          while( server == client ):
                server = random.choice(host_list)
          if(_bw==None):bw=str(random.randint(50,200))+'k'
          if(_period==None):period=random.randint(5,30)
          self.iperf_single(hosts = [client, server], udpBw=bw, period= period, port=base_port)
          #sleep(.05)
      ...
      sleep(period)
      print "test has done"
    def iperfPeer(self,_bw, _period):
      base_port = 7001
      client_list = [h for h in self.hosts if (int(h.name[1:])<=4)]
      server_list = [h for h in self.hosts if (int(h.name[1:])>4)]
      bw=_bw
      period=_period
      while(True):
      ...
      sleep(period)
      print "test has done"
    def iperfRead(self,fname='sw7'):
      base_port = 7001
      fname='/home/yg/ryucode/testflow/'+fname
      try:
        fobj=open(fname,'r')
      except IOError,e:
        ...
        for line in lines:
          ...
          bw=items[4]
          sleep(interval)
          client = self.hosts[src]
          server = self.hosts[dst]
          bw=bw+'k'
          self.iperf_single(hosts = [client, server], udpBw=bw, period= period, port=base_port)
          #port ++
          base_port += 1
