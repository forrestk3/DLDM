...
class CLI( Cmd ):
    ...
    def do_iperfrandom( self,line):
        args = line.split()
        if len(args) == 0:
            print 'random flow bw and period'
            self.mn.iperfRandom(None,None)
        elif len(args) == 1:
            udpBw = args[ 0 ]
            print 'random flow bw=%s' % (udpBw)
            self.mn.iperfRandom(udpBw,None)
        elif len(args) == 2:
            udpBw = args[ 0 ]
            period = args[ 1 ]
            print 'random flow bw=%s period=%s' % (udpBw,period)
            err = False
            self.mn.iperfRandom(udpBw, float(period))
        else:
            error('invalid number of args: iperfrandom\n' +
               'examples:iperfrandom 100k 120\n')
    def do_iperfpeer( self,line):
        args = line.split()
        if len(args) == 0:
            print 'random flow bw and period'
            self.mn.iperfPeer(None,None)
        elif len(args) == 1:
            udpBw = args[ 0 ]
            print 'random flow bw=%s' % (udpBw)
            self.mn.iperfPeer(udpBw,None)
        elif len(args) == 2:
            udpBw = args[ 0 ]
            period = args[ 1 ]
            print 'random flow bw=%s period=%s' % (udpBw,period)
            err = False
            self.mn.iperfPeer(udpBw, float(period))
        else:
            error('invalid number of args: iperfpeer\n' +
               'examples:iperfpeer 100 120\n')
    def do_iperfread(self,filename):
        print filename 
        filename=filename.strip('\n')
        self.mn.iperfRead(filename)
