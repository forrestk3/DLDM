...
#	_CONTEXTS={'dpset':dpset.DPSet}
	...
	def __init__(self,*args,**kwargs):
		...
		#self.topoFile='/home/husp/ryucode/topo/topo-7sw.csv'
		self.topoFile='/home/husp/ryucode/topo/topo-10sw-dc.csv'
...
#		print dir(msg)
		...
		outport=self.getOutport(dpid,flow)
		data=None
...
#					print int(round(time.time() * 1000))
	...
	def _record(self):
		...
		while(True):
			...
			print ("%s %s %s %s %s %s %s %s\n" % (cnt,y0_new,y1_new,y2_new,y3_new,y4_new,y5_new,self.energyTotal))
	...
	def get_load_energy(self):
		...
		return mean,load_var,energy,throughput,
	def scheduleFlows(self,cookie):
		print 'scheduleFlows'
		flow=self.flows[cookie]
		src=flow.src
		dst=flow.dst
		#add max fit---------------------------------------------
		path=self.getNewPath(src,dst,cookie)
		#modify flow entry
		if(len(path)>0 and flow.path!=path):
			self.change_path(cookie,flow.path,path,src,dst)
			self.getWeight1(cookie)
		pass
	...
	def getRerouteMatrix(self,typeTopo,cookie):
		...
		print favorNet
		return favorNet
	def getNewPath(self,src,dst,cookie):
		print 'getNewPath',src,dst
		s=self.hosts[src].tdpid
		t=self.hosts[dst].tdpid
		if(s==t):return [s]
		paths={}
		typeTopo='active'
		actFavorMat=self.getRerouteMatrix(typeTopo,cookie)
		if(actFavorMat==None):
			print 'can not find new path'
			return []
			...
			for i in range(mpath):
				...
				if(len(path)>0):
					if((tuple(path) in paths.keys())):
						...
						paths[tuple(path)]=1
		typeTopo='global'
		gloFavorMat=self.getRerouteMatrix(typeTopo,cookie)
		if(gloFavorMat==None):
			print 'can not find new path'
			return []
		...
		if(arrive==False):
			print 'can not find new path'
			return []
		bw=self.flows[cookie].bw
		return self.evalue(paths,bw)
	...
	def score4(self,paths,prepath,pathsNewEnergy):
		...
		for path in paths.keys():
			...
			for i in range(len(path)-1):
				...
				if(self.core_links[edgeID].ratio>rmax):rmax=self.core_links[edgeID].ratio
				if(self.core_links[edgeID].ratio<rmin):rmin=self.core_links[edgeID].ratio
			left=(1-(rmax-rmin)+c0)*paths[path]/mpath
			rght=((maxEnergy-pathsNewEnergy[path])/(c0+maxEnergy-minEnergy))*(1-paths[path]/mpath)
